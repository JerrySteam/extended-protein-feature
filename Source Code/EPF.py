...
dataset_type = "plant/Ara_PsyGorHpa" #bacteria/bacillus, virus/human_sars_cov2, virus/human_hiv1, parasite/plasmodium
...
print("Performing protein sequence feature encoding 1...")
...
# perform the feature encoding technique (Note: This operations takes time)
print("Performing protein sequence feature encoding 2...")
...
# combine Independent and Merged protein features
...
print("Performing feature extraction with FI and PCA...")
hpi_data = pd.read_csv(main_path + "preprocessed/" + dataset_type_with_underscore + ".csv", index_col=[0])
# hpi_data = pd.read_csv(main_path + "preprocessed/" + dataset_type_with_underscore + ".csv")
...
#     # Create balance dataset: If interacting > non-interacting samples, randomly select interacting samples
#     rand_int_hpi_data = int_hpi_data.sample(len(nonint_hpi_data))
#
#     # merge the int random samples with the nonint samples to form a balance dataset
#     bal_hpi_data = rand_int_hpi_data.append(nonint_hpi_data, ignore_index=True)
#     hpi_data = bal_hpi_data
#
# else:
#     # Create balance dataset: If non-interacting > interacting samples, randomly select non-interacting samples
...
# Random forest feature importance 400 = 82.15, for remove 0 fi 81.01
X_train_fi, X_test_fi, y_train_fi, y_test_fi = train_test_split(X_hpi, y_hpi, test_size=0.30)
X_train_fi = np.nan_to_num(X_train_fi) # replace NAN. INFINITY to Zero

# A random forest classifier is fitted to compute the feature importances.
feature_names = [f"feature {i}" for i in range(1, X_hpi.shape[1]+1)]
forest = RandomForestClassifier(random_state=0)
forest.fit(X_train_fi, y_train_fi)
...
forest_importances = pd.Series(importances, index=feature_names).loc[lambda x : x >= 0.001] #virus, parasite, plant
# forest_importances = pd.Series(importances, index=feature_names).loc[lambda x : x != 0.0] #bacteria
important_features = forest_importances.index.tolist() # get important features as a list
X_hpi.columns = feature_names # rename original data features
X_hpi_fimp = X_hpi.loc[:, important_features] # extract dataset based on important feature columns
...
# plt.xticks([], [])
# plt.plot(forest_importances[0:800], linestyle="", marker="*", label='Independent')
# plt.plot(forest_importances[800:1201], linestyle="", marker=".", label='Merged')
# plt.legend(loc="upper right")
# plt.show()

# computing pca = Pr
start_time = time.time()
no_of_comp = 95 #600 #95 #206 #parasite:92; bacteria:88,600; virus:89; plant:95
pca = PCA(n_components=no_of_comp)
X_hpi_fimp = np.nan_to_num(X_hpi_fimp) # replace NAN. INFINITY to Zero
# pca = PCA(n_components=0.85)
X_pca = pca.fit_transform(X_hpi_fimp)
# print(f"Number of components selected: {pca.n_components_}")
print(f"Explained variance ratio: {pca.explained_variance_ratio_.cumsum()[no_of_comp-1]}")
feature_names = [f"PC {i}" for i in range(1, X_pca.shape[1]+1)]
X_hpi_pca = pd.DataFrame(data = X_pca, columns = feature_names)
elapsed_time = time.time() - start_time
print(f"Elapsed time to compute the PCA: {elapsed_time:.3f} seconds")
hpi_data = pd.concat([X_hpi_pca, y_hpi], axis=1)
print(hpi_data.shape)
